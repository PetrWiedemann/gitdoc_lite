...
Až jsou všichni spokojení, správce klikne na velké zelené tlačítko "Merge Pull Request". GitHub u tohoto tlačítka ale nabízí 3 různé strategie sloučení:
1. **Create a merge commit:** Klasický `git merge` – zanechá "vlakové nádraží" a v historii zůstanou úplně všechny vaše drobné commity.
2. **Squash and merge:** Velmi oblíbené. Vezme všech 50 vašich drobných testovacích commitů, slisuje je do jednoho obrovského commitu a ten vloží do `main`. Udržuje to historii neuvěřitelně čistou.
3. **Rebase and merge:** Vezme vaše commity a rebasuje je na `main` bez vytvoření merge commitu (pokud to jde čistě).

> [!TIP] Pull Requesty ve firmách
> Ačkoliv firmy nepoužívají forky (všichni vývojáři sdílí jeden repozitář), koncept Pull Requestů se používá i tam! Vývojář pošle svou větev na firemní GitHub (`git push origin feature-A`) a rovnou z ní vytvoří PR do větve `main`. Slouží to jako povinné "razítko kvality" – kód se nedostane do produkce, dokud ho v PR nezkontrolují alespoň dva další kolegové. Často se používají i tzv. **Draft PR**, což je žádost, která říká "Zatím na tom dělám, jen vám ukazuji kód, neslučujte to".
...
*   **GitHub Pages:** Umožňuje zdarma hostovat vaši dokumentaci (jako je tato) přímo z repozitáře. Historicky to vyžadovalo speciální větev `gh-pages`, dnes ale můžete Pages nastavit z libovolné větve, složky `/docs`, nebo nejlépe vše plně zautomatizovat přes GitHub Actions.
*   **Branch Protection:** Administrátoři mohou uzamknout větev `main`. Nikdo do ní pak nemůže komitovat přímo (`git push` spadne s chybou). Jediná cesta do ní je přes schválený Pull Request a zelené Unit Testy.
*   **Releases:** Možnost zabalit konkrétní Git Tag (např. `v1.0.0`) do oficiálního vydání, ke kterému můžete připojit zkompilované binární soubory a changelog (seznam změn).
